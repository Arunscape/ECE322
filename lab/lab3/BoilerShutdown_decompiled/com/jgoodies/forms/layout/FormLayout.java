...
public final class FormLayout implements LayoutManager2, Serializable
{
    ...
    private void shiftComponentsHorizontally(final int columnIndex, final boolean remove) {
        ...
        for (final Map.Entry entry : this.constraintMap.entrySet()) {
    ...
    private void adjustGroupIndices(final int[][] allGroupIndices, final int modifiedIndex, final boolean remove) {
        final int offset = remove ? -1 : 1;
    ...
    public void layoutContainer(final Container parent) {
        synchronized (parent.getTreeLock()) {
            ...
            final int[] x = this.computeGridOrigins(parent, totalWidth, insets.left, this.colSpecs, this.colComponents, this.colGroupIndices, this.minimumWidthMeasure, this.preferredWidthMeasure);
            final int[] y = this.computeGridOrigins(parent, totalHeight, insets.top, this.rowSpecs, this.rowComponents, this.rowGroupIndices, this.minimumHeightMeasure, this.preferredHeightMeasure);
    ...
    private Dimension computeLayoutSize(final Container parent, final Measure defaultWidthMeasure, final Measure defaultHeightMeasure) {
        synchronized (parent.getTreeLock()) {
            this.initializeColAndRowComponentLists();
            final int[] colWidths = this.maximumSizes(parent, this.colSpecs, this.colComponents, this.minimumWidthMeasure, this.preferredWidthMeasure, defaultWidthMeasure);
            final int[] rowHeights = this.maximumSizes(parent, this.rowSpecs, this.rowComponents, this.minimumHeightMeasure, this.preferredHeightMeasure, defaultHeightMeasure);
            final int[] groupedWidths = this.groupedSizes(this.colGroupIndices, colWidths);
            final int[] groupedHeights = this.groupedSizes(this.rowGroupIndices, rowHeights);
            final int[] xOrigins = this.computeOrigins(groupedWidths, 0);
            final int[] yOrigins = this.computeOrigins(groupedHeights, 0);
            final int width1 = sum(groupedWidths);
            final int height1 = sum(groupedHeights);
            int maxWidth = width1;
            int maxHeight = height1;
            final int[] maxFixedSizeColsTable = this.computeMaximumFixedSpanTable(this.colSpecs);
            final int[] maxFixedSizeRowsTable = this.computeMaximumFixedSpanTable(this.rowSpecs);
            for (final Map.Entry entry : this.constraintMap.entrySet()) {
                final Component component = entry.getKey();
                final CellConstraints constraints = entry.getValue();
                if (!this.takeIntoAccount(component, constraints)) {
    ...
    private int[] computeGridOrigins(final Container container, final int totalSize, final int offset, final List formSpecs, final List[] componentLists, final int[][] groupIndices, final Measure minMeasure, final Measure prefMeasure) {
        final int[] minSizes = this.maximumSizes(container, formSpecs, componentLists, minMeasure, prefMeasure, minMeasure);
        final int[] prefSizes = this.maximumSizes(container, formSpecs, componentLists, minMeasure, prefMeasure, prefMeasure);
        final int[] groupedMinSizes = this.groupedSizes(groupIndices, minSizes);
        final int[] groupedPrefSizes = this.groupedSizes(groupIndices, prefSizes);
        final int totalMinSize = sum(groupedMinSizes);
        final int totalPrefSize = sum(groupedPrefSizes);
        final int[] compressedSizes = this.compressedSizes(formSpecs, totalSize, totalMinSize, totalPrefSize, groupedMinSizes, prefSizes);
        final int[] groupedSizes = this.groupedSizes(groupIndices, compressedSizes);
        final int totalGroupedSize = sum(groupedSizes);
        final int[] sizes = this.distributedSizes(formSpecs, totalSize, totalGroupedSize, groupedSizes);
    ...
    private int[] maximumSizes(final Container container, final List formSpecs, final List[] componentLists, final Measure minMeasure, final Measure prefMeasure, final Measure defaultMeasure) {
    ...
    private int[] compressedSizes(final List formSpecs, final int totalSize, final int totalMinSize, final int totalPrefSize, final int[] minSizes, final int[] prefSizes) {
    ...
    private int[] groupedSizes(final int[][] groups, final int[] rawSizes) {
        ...
        for (int group = 0; group < groups.length; ++group) {
            ...
            for (int j = 0; j < groupIndices.length; ++j) {
                final int index = groupIndices[j] - 1;
    ...
    private int[] distributedSizes(final List formSpecs, final int totalSize, final int totalPrefSize, final int[] inputSizes) {
